...
var banner = {
  init: function(){
    ...
    if ($(window).width() >= 768) {
      ...
      if(jQuery('a.__seguno-banner-container').length > 0){
      	totalHeaderHeight = totalHeaderHeight - jQuery('a.__seguno-banner-container').height();
      }

      if ($('body').hasClass('index') || $('body').hasClass('feature_image') || $('.shopify-section.banner-template-section').length || $('.shopify-section.details-section').length || $('.collection-banner-template-section').length){

        var $banner = $('.banner-section').first();
        var $draggableSection = $('.index-sections .shopify-section, .page-details-section .page-details-block').first();

        if ($draggableSection.length) {
          var $firstSection = $draggableSection.children().first();
        } else {
          var $firstSection = $banner;
        }

        $firstSection.attr('style', '');

        if ((!$firstSection.hasClass('wide_display--false') && $firstSection.find('.slider').length) || !$firstSection.hasClass('wide_display--false') && $firstSection.find('.flexslider').length) {

          if ($('.header').hasClass('behind-menu--false') && !$firstSection.hasClass('wide_display--false')){
            $firstSection.css('padding-top', totalHeaderHeight - 25);
            $('.header').parent().removeClass('feature_image');
            $('.header').addClass('header_bar');
          } else if ($('.header').hasClass('behind-menu--true')){
            $('.header').parent().addClass('feature_image');
            $('.header').removeClass('header_bar js-no-change');
          } else {
            $('.header').parent().addClass('feature_image');
            $('.header').removeClass('header_bar js-no-change').addClass('header_bar behind-menu--false');
            $('.header').removeClass('header_bar js-no-change behind-menu--false').addClass('behind-menu--true');
          }

        } else {
          if ($('.header').hasClass('behind-menu--false')){
            $('.header').addClass('header_bar');
          } else {
            $('.feature_image').removeClass('feature_image');
            $('.header').removeClass('behind-menu--true').addClass('header_bar behind-menu--false');
          }
        }
      }
    }
  ...
  unload: function($target){
    ...
    $target.children().first().attr('style', '');
  }
}
...
$(function() {
  	/*
	jQuery("body").html(jQuery("body").html().replace(/\®/g, "<sup>&reg;</sup>"));
	jQuery("body").html(jQuery("body").html().replace(/\&trade;/g, "<sup>&trade;</sup>"));
	*/
  	jQuery("#go-to-trustpilot-reviews > span").click(function() {
	    jQuery('html, body').animate({
	        scrollTop: jQuery("#tp-shopify-product-reviews").offset().top - 150
	    }, 2000);
	});
  ...
  function resize() {
        ...
        if(windowWidth > 768) {
            jQuery("img.responsive__image__changer").each(function() {
                if (jQuery(this).attr("data-src-desktop")) {
                    jQuery(this).attr("src", jQuery(this).attr("data-src-desktop"));
                };
            });
        }else{
            jQuery("img.responsive__image__changer").each(function() {
        ...
        $('.homepage_slider').each((i, el) => destroySlider($(el)));
        slideshow.init(true);
        banner.init();
    }
...
//Free Over $60 Product Promotion
function addFreeProduct(cart) {
  var freeIdFound = false;
  var overPrice = 6000;
  var freeId = 8029849976884;
  var enableFreeId = true;
  var founditem;
  ...
  if (freeIdFound) {
  	totalPrice = totalPrice - (founditem.price * founditem.quantity);
  }

  if(totalPrice > overPrice && !freeIdFound) {
      $.post('/cart/add.js', {
        id: freeId
      });
  } else if(totalPrice < overPrice && freeIdFound) {
      $.post('/cart/update.js', 'updates['+freeId+']=0');
  }
}
...
function refreshCart(cart) {
  ...
  if ($cartBtn) {
    ...
    var freeId = 25223099401; //Free Product Promotion
    var freeItem = false; 
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        ...
        cart_items_html += '<div class="price"><span class="money">' + Shopify.formatMoney(item.line_price, $('body').data('money-format')) + '</span></div></div></a></li>';
      });
      ...
      cart_items_html += '<li class="mm-label">' +
        ...
        $('#cart_form .money').empty().append(Shopify.formatMoney(cart.total_price, $cart.data('money-format')));

        
          var cart_note = cart.note ? cart.note : '';
          cart_items_html += '<textarea id="note" name="note" rows="2" placeholder="Order Notes" class="clearfix">'+cart_note+'</textarea>';
          ...
          cart_items_html += '<aside class="tos tos_checkbox"><input type="checkbox" class="tos_agree" id="sliding_agree" required /><label class="tos_label" for="sliding_agree"> ' +
            "\u003cp\u003eI Agree with the \u003ca href=\"\/pages\/terms-of-use\" title=\"Terms of Use\"\u003eTerms \u0026amp; Conditions\u003c\/a\u003e\u003c\/p\u003e" + '</label></aside>';
